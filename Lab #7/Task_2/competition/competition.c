...
int lock(int fd, long no) {
    ...
    fl.l_whence = SEEK_SET;//смещ от начала файла
    fl.l_type = F_WRLCK;//блокировка для записи
...
int unlock(int fd, long no) {
    ...
    fl.l_type = F_UNLCK;//снятие блокировки
...
void print_lock(int fd, long no) {
    ...
    if (fcntl(fd, F_GETLK, &fl) == -1) {
        if (errno == EACCES || errno == EAGAIN) {
            ...
            fprintf(stderr, "[ERROR]: Unhandled error while unlocking: %s\n", strerror(errno));
        }
    }
