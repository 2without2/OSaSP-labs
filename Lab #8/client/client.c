...
int executeFile(const char* path, int client_fd, char* cur_path) {
    ...
    if ((fd = open(path, O_RDONLY)) == -1) {
        fprintf(stdout,"CLIENT: [ERROR]: Can't open file \n");
        return -1;
    }
...
void runClient(char* addr, long port) {
    ...
    while (1) {
        ...
        // Receive and print server response
        long b = recv(client_fd, buffer, BUFFER_SIZE * sizeof(char), 0);//принять
        if (b == -1) {
            fprintf(stderr, "CLIENT: [ERROR]: Error when reading server data\n");
            close(client_fd);
            break;
...
int main(int argc, char* argv[]) {
    ...
    if (!inet_pton(AF_INET, addr, &(sa.sin_addr))) { //преобразуем аддрес в бинарный вид
        fprintf(stderr, "CLIENT: [ERROR]: Can't parse ip address\n");
    }
    long port = strtol(argv[2], NULL, 10);//в лонг
    runClient(addr, port);
}